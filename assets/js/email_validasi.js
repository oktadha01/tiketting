	...
	function validateEmail() {
		if (input.value.length >= minLength && input.value.length <= maxLength) {
			...
			if (input.value.indexOf("@") !== -1) {
				...
				if (atIndex >= 0) {
					...
					// check more than 2 dots after "@"
						...
						$("#email").removeClass("invalid");
						$(".valid_info")
							.text('The address entered is invalid. Too many dots after "@"')
							.css("color", "red");
						$("#email").addClass("invalid");
						let infoCallback = function () {
							$(".valid_info")
			...
			// check duplicates
				...
				$("#email").removeClass("invalid");
				$(".valid_info")
					.text("The address entered is invalid. Too many identical characters")
					.css("color", "red");
				$("#email").addClass("invalid");
				let infoCallback = function () {
					$(".valid_info")
				...
				$("#email").on("keypress", function (event) {
					...
					if (
						...
						if (
							value.indexOf("@") !== -1 &&
							value.indexOf(".", value.indexOf("@")) !== -1 &&
							value.indexOf(".", value.indexOf(".", value.indexOf("@") + 1)) !==
							-1 &&
							!/[a-zA-Z]/.test(key)
						) {
							event.preventDefault();
